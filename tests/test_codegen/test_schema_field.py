...
def test__field_existed_as__extras_are_added(
    ...
    v1 = create_local_simple_versioned_schemas(
        schema(latest_with_empty_classes.EmptySchema).field("foo").existed_as(type=int, info=Field(deflolt="hewwo")),
    )
...
def assert_field_had_changes_apply(
    ...
    assert getattr(getattr(v1, model.__name__).__fields__["foo"].field_info, attr) == attr_value
