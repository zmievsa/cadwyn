...
def test__endpoint_existed__with_several_methods(router: VersionedAPIRouter):
    @router.only_exists_in_older_versions
    @router.get("/test")
    async def test_endpoint():
        raise NotImplementedError
    ...
    routes_2000, routes_2001 = create_versioned_api_routes(
        router,
        endpoint("/test", ["GET", "DELETE"]).existed,
    )

    assert len(routes_2000) == 3
    assert routes_2000[0].endpoint.func == test_endpoint_post
    assert routes_2000[1].endpoint.func == test_endpoint
    assert routes_2000[2].endpoint.func == test_endpoint_delete
...
def test__router_generation__updating_request_models(
    ...
    assert len(routes_2000) == len(routes_2001) == 1
    assert (
            routes_2000[0].dependant.body_params[0].annotation == dict[
        str, list[schemas_2000.SchemaWithOnePydanticField]]
    )
    assert (
            routes_2001[0].dependant.body_params[0].annotation == dict[
        str, list[schemas_2001.SchemaWithOnePydanticField]]
    )
