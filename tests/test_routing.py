...
from typing import Annotated, Any, Awaitable, Callable, TypeAlias, cast, get_args
...
from tests.test_codegen import generate_test_version_packages  # TODO: It's bad to import between tests like that
from universi import Versions, VersionedAPIRouter
...
def create_versioned_copies(
    ...
    *instructions: AlterSchemaInstruction | AlterEndpointSubInstruction | AlterEnumSubInstruction,
...
def create_versioned_api_routes(
    ...
    *instructions: AlterSchemaInstruction | AlterEndpointSubInstruction | AlterEnumSubInstruction,
    ...
    routers = create_versioned_copies(router, *instructions, latest_schemas_module=latest_schemas_module)
...
def test__router_generation__forgot_to_generate_schemas__error(router: VersionedAPIRouter):
    with pytest.raises(
...
def test__endpoint_didnt_exist(router: VersionedAPIRouter, test_endpoint: Endpoint):
    routes_2000, routes_2001 = create_versioned_api_routes(router, endpoint(test_endpoint).didnt_exist)
...
def test__endpoint_existed(router: VersionedAPIRouter):
    ...
    routes_2000, routes_2001 = create_versioned_api_routes(router, endpoint(test_endpoint).existed)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("generate_unique_id_function", lambda api_route: api_route.endpoint.__name__),  # pragma: no cover
    ],
)
def test__endpoint_had(router: VersionedAPIRouter, attr: str, attr_value: Any, test_endpoint: Endpoint):
    routes_2000, routes_2001 = create_versioned_api_routes(router, endpoint(test_endpoint).had(**{attr: attr_value}))
...
def test__endpoint_only_exists_in_older_versions__endpoint_is_not_a_route__error(
    router: VersionedAPIRouter, test_endpoint: Endpoint
):
    with pytest.raises(LookupError, match=re.escape("Route not found on endpoint: 'test2'")):
...
def test__router_generation__non_api_route_added(router: VersionedAPIRouter, test_endpoint: Endpoint):
...
def test__router_generation__creating_a_synchronous_endpoint__error(router: VersionedAPIRouter):
    @router.get("/test")
    def test():
        raise NotImplementedError
...
def test__router_generation__changing_a_deleted_endpoint__error(router: VersionedAPIRouter):
    @router.only_exists_in_older_versions
    @router.get("/test")
    async def test():
        raise NotImplementedError
    ...
    with pytest.raises(
        RouterGenerationError,
        match=re.escape("Endpoint 'test' you tried to delete in 'VersionChange' doesn't exist in new version"),
    ):
        create_versioned_copies(router, endpoint(test).had(description="Hewwo"))


def test__router_generation__deleting_a_deleted_endpoint__error(router: VersionedAPIRouter):
    @router.only_exists_in_older_versions
    @router.get("/test")
    async def test():
        raise NotImplementedError
...
def test__router_generation__re_creating_an_existing_endpoint__error(
    ...
    with pytest.raises(
        RouterGenerationError,
        match=re.escape("Endpoint 'test' you tried to re-create in 'VersionChange' already existed in newer versions"),
    ):
...
def test__router_generation__re_creating_a_non_endpoint__error(router: VersionedAPIRouter):
    async def test():
        raise NotImplementedError

    with pytest.raises(
        RouterGenerationError,
        match=re.escape("Endpoint 'test' you tried to re-create in 'VersionChange' wasn't among the deleted routes"),
    ):
        create_versioned_copies(router, endpoint(test).existed)


def test__router_generation__non_api_route_added_with_schemas(router: VersionedAPIRouter, test_endpoint: Endpoint):
    @router.websocket("/test2")
    async def test_websocket():
        raise NotImplementedError

    generate_test_version_packages(router)
    routers = create_versioned_copies(router, endpoint(test_endpoint).didnt_exist, latest_schemas_module=latest)
    assert len(routers[date(2000, 1, 1)].routes) == 1
    assert len(routers[date(2001, 1, 1)].routes) == 2
    route = routers[date(2001, 1, 1)].routes[0]
    assert isinstance(route, APIRoute)
...
def test__router_generation__updating_response_model(router: VersionedAPIRouter):
    @router.get("/test", response_model=dict[str, list[latest.SchemaWithOnePydanticField]])
    async def test():
        raise NotImplementedError

    instruction = schema(latest.SchemaWithOneIntField, field("foo").had(type=list[str]))
    schemas_2000, schemas_2001 = generate_test_version_packages(router, instruction)

    routes_2000, routes_2001 = create_versioned_api_routes(router, instruction, latest_schemas_module=latest)
    assert len(routes_2000) == len(routes_2001) == 1
...
def test__router_generation__updating_request_models(router: VersionedAPIRouter):
    @router.get("/test")
...
def test__router_generation__updating_request_depends(router: VersionedAPIRouter):
    ...
    def dependency2(dep: Annotated[latest.StrEnum, Depends(sub_dependency2)] = latest.StrEnum.a):
        return dep
    ...
    generate_test_version_packages(router, instruction)
