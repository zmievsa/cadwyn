...
from cadwyn._codegen.plugins.class_migrations import class_migration_plugin
from cadwyn._codegen.plugins.class_rebuilding import ClassRebuildingPlugin
from cadwyn._codegen.plugins.class_renaming import ClassRenamingPlugin
from cadwyn._codegen.plugins.import_auto_adding import ImportAutoAddingPlugin
from cadwyn._codegen.plugins.latest_version_aliasing import LatestVersionAliasingPlugin
...
def generate_code_for_versioned_packages(
    ...
    *,
    ...
    codegen_plugins: Sequence[CodegenPlugin] = (
        LatestVersionAliasingPlugin(),
        ClassRebuildingPlugin(),
        ClassRenamingPlugin(),
        ImportAutoAddingPlugin(),
    ),
    migration_plugins: Sequence[MigrationPlugin] = (class_migration_plugin,),
):
    """
    Args:
...
def _apply_per_node_plugins(
    ...
    for node in parsed_file.body:
        node_type = type(node)
        for plugin in plugins:
            if isinstance(node_type, plugin.node_type):
