...
class _ModelFieldLike:
    ...
    def _attributes_set(self):
        return self.field_info._attributes_set
    ...
    def change_field(  # noqa: C901
        ...
        for attr_name in alter_schema_instruction.field_changes.__dataclass_fields__:
            ...
            if attr_value is not Sentinel:
                # TODO: Add this check for metadata fields too!
                ...
                if hasattr(model_field.annotation, attr_name) and current_field_is_constrained_type:
                    ...
                    if PYDANTIC_V2 and attr_name in FieldInfo.metadata_lookup:
                        field_info.metadata.extend(FieldInfo._collect_metadata({attr_name: attr_value}))
                    else:
                        setattr(field_info, attr_name, attr_value)
                    if PYDANTIC_V2:
                        field_info._attributes_set[attr_name] = attr_value
...
def _generate_field_ast(
    ...
    passed_attrs = {
        attr: _get_attribute_from_field_info(field, attr) for attr in _get_passed_attributes_to_field(field.field_info)
    }
...
def _get_attribute_from_field_info(field: _ModelFieldWrapper, attr: str) -> Any:
    if PYDANTIC_V2:
        return field.field_info._attributes_set[attr]
    else:
        field_info = field.field_info
    field_value = getattr(field_info, attr, Sentinel)

    if field_value is Sentinel:
        # TODO: Such gettattrs should probably be done using a wrapper
        extra = getattr(field_info, EXTRA_FIELD_NAME)
        field_value = (extra or {}).get(attr, Sentinel)
...
def _get_passed_attributes_to_field(field_info: FieldInfo):
    if PYDANTIC_V2:
        # PydanticV2 always adds frozen to attributes_set which makes serialization weird
        attributes_set = field_info._attributes_set.copy()
        if attributes_set.get("frozen", ...) is None:
            attributes_set.pop("frozen")
        attributes_to_look_at = attributes_set.keys() & _empty_field_arg_names

        for attr_name in sorted(attributes_to_look_at):
            yield attr_name
